...
int main(int argc, char* argv[])
{
  ...
  // THE HACK
  System<Molecule> myNewSystem;
  myNewSystem.setLattice(mySystem.lattice());
  myNewSystem.setName(mySystem.name());
  for(size_t i = 0; i < mySystem.size(); ++i)
  {
    bool erase = true;
    for(size_t j = 0; j < 10; ++j)
      if(list[j] == i) erase = false;
    if(!erase) myNewSystem.add(mySystem[i]);
  }
  mySystem = myNewSystem;
  numSoluteResidues = mySystem.size();
  // END OF HACK
