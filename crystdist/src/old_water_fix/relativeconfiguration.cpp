...
// Uncomment for the temporary water fix
#define WATER_FIX
                                      ...
                                      Lattice const &lattice)
{
          ...
          (first.type == PLANAR_SYMMETRIC && second.type == PLANAR_SYMMETRIC))
  {
    // Axis vs. axis:
    //
    // Bond orientation and relative orientation are defined line in the 
...
Real const squareDeviation(RelativeConfiguration const &conf1, RelativeConfiguration const &conf2)
{
  ...
  {
#ifdef WATER_FIX
    // Temporary fix for water - lines commented are the original code
    Vector3D vec1 = vector(conf1.bondOrientation);
    Vector3D vec2 = vector(conf2.bondOrientation);
    Real dotProduct = vec1*vec2;
    Real dotProduct2 = vec1.x*vec2.x - vec1.y*vec2.y - vec1.z*vec2.z;
    if(dotProduct < dotProduct2) dotProduct = dotProduct2;
    norm2 += 1 - dotProduct;
    Quaternion quat1 = conf1.relativeOrientation;
    Quaternion quat2 = conf2.relativeOrientation;
    Quaternion q_i(0.0, 1.0, 0.0, 0.0);
    dotProduct = dot(quat1, quat2);
    dotProduct2 = dot(-q_i*quat1, quat2);
    if(fabs(dotProduct2) > fabs(dotProduct)) dotProduct = dotProduct2;
    dotProduct2 = dot(quat1*q_i, quat2);
    if(fabs(dotProduct2) > fabs(dotProduct)) dotProduct = dotProduct2;
