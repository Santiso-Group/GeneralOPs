                                      ...
                                      Lattice const &lattice)
{
  ...
  if(first.type == GENERAL && second.type == GENERAL)
  {
    ...
    // Water temporary "fix" 02-23-10
#ifdef WATER_FIX
    if(bondOrientation.y < 0.0) bondOrientation.y = -bondOrientation.y;
    if(bondOrientation.z < 0.0) bondOrientation.z = -bondOrientation.z;
#endif
    relativeOrientation = ~first.orientation*second.orientation;
#ifdef WATER_FIX
    Matrix3D rotMat = rotationMatrix(relativeOrientation);
    for(size_t i = 0; i < 3; ++i)
    {
      if(rotMat(1, i) < 0) 
      {
        rotMat(1, i) = -rotMat(1, i);
	rotMat(2, i) = -rotMat(2, i);
      }
    }
    relativeOrientation = Quaternion(rotMat);
#endif
  }
